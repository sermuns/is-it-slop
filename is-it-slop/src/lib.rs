...
use std::io::{StdoutLock, Write as _, stdout};
...
use crate::github::{
    GitHubProject, fetch_gitignore, fetch_repo_details, find_gitignored_sussy_files,
    find_sussy_files,
};

struct StdoutAndString<'a> {
    string: String,
    stdout: StdoutLock<'a>,
}

impl Write for StdoutAndString<'_> {
    fn write_str(&mut self, s: &str) -> fmt::Result {
        self.string.push_str(s);
        self.stdout.write_all(s.as_bytes()).ok();
        Ok(())
    }
}
...
pub struct SlopReport {
    pub text: String,
    pub slop_score: usize,
}

pub async fn generate_slop_report(
    github_project: &GitHubProject,
    git_ref: &str,
) -> color_eyre::Result<SlopReport> {
    let mut tee = StdoutAndString {
        string: String::new(),
        stdout: stdout().lock(),
    };
    ...
    writeln!(tee, "it is {:#} old", duration_since_creation);

    const YOUNG_AGE_HOURS: f64 = 24.;
    match duration_since_creation.total((jiff::Unit::Hour, &now_utc)) {
        ...
        Err(e) => {
            writeln!(tee, "error during age calculation: {:?}", e);
        }
        _ => (),
    }

    let cargo_toml = fetch_cargo_toml(&github_project, git_ref, &client).await?;
    ...
    let sussy_files_present = find_sussy_files(&github_project, git_ref, &client).await;

    let gitignore = fetch_gitignore(&github_project, git_ref, &client).await?;
